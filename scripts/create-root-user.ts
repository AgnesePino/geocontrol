...
export async function createRootUser() {
  ...
  try {
    ...
    if (existingUser) {
      ...
      // Use environment variable for the root password. If not provided, generate a random one.
      const rootPassword = process.env.ROOT_PASSWORD || crypto.randomBytes(12).toString("hex");
      await userRepo.createUser("root", rootPassword, UserType.Admin);
      logInfo("Root user created successfully. Password set from ROOT_PASSWORD env var or auto-generated.");
      if (!process.env.ROOT_PASSWORD) {
        logWarn(`Generated root password: ${rootPassword}`);
      }
    }
  } catch (error) {
